...
| 9.34 | Reload a server that has been up for hours | On a machine where conch has run for hours with agents in panes, several watched projects and a TUI attached, `conch server reload` onto a fresh build; watch `conch status` from another terminal | The server comes back within a second or two with its panes; it never sits in a state where the socket is there but nothing answers. If it does: `conch status` says "not answering", running `conch` again starts a fresh server, and the old process's stacks (`sample PID`) go in the bug report | ☐ seen once on macOS: stuck in runtime_BeforeExec, recovered by removing the socket |
